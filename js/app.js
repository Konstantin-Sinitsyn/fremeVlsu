(() => {
 ...
 /*! smooth-scroll v16.1.3 | (c) 2020 Chris Ferdinandi | MIT License | http://github.com/cferdinandi/smooth-scroll */            window.Element && !Element.prototype.closest && (Element.prototype.closest = function(e) {
        ...
        802: (__unused_webpack___webpack_module__, __unused_webpack___webpack_exports__, __webpack_require__) => {
            "use strict";
            const flsModules = {};
            function isWebp() {
            ...
            function initSliders() {
                if (document.querySelector(".committees__slider")) new core(".committees__slider", {
            ...
            const historyUp = document.getElementById("historyUp");
            if (historyUp) window.addEventListener("scroll", (function() {
                if (window.scrollY > 700) historyUp.classList.add("shadow"); else historyUp.classList.remove("shadow");
            }));
            const switchLanguageBtn = document.querySelector(".language");
            const arrayElement = document.querySelectorAll("[data-translate]");
            if ("#EN" === window.location.hash) for (let element of switchLanguageBtn.children) {
                element.classList.remove("_active");
                if ("#EN" === element.hash) {
                    element.classList.add("_active");
                    let language = en_namespaceObject;
                    initTranslation(language);
                }
            }
            switchLanguageBtn.addEventListener("click", (event => {
                for (let element of switchLanguageBtn.children) element.classList.remove("_active");
                event.target.classList.add("_active");
                let language = "RU";
                language === event.target.innerHTML ? language = ru_namespaceObject : language = en_namespaceObject;
                initTranslation(language);
            }));
            function initTranslation(language) {
                translationText(arrayElement, language);
            }
            function translationText(arr, language) {
                arr.forEach((element => {
                    element.innerHTML = language[element.getAttribute("data-translate")];
                }));
            }
            function map(n) {
                ymaps.ready(init);
                function init() {
                    var myMap = new ymaps.Map("map", {
                        controls: [],
                        center: [ 56.145574, 40.371524 ],
                        zoom: 16
                    });
                    observeEvents(myMap);
                    let myPlacemark = new ymaps.Placemark([ 56.145574, 40.371524 ], {
                        balloonContentHeader: "Кафедра ЭПБС",
                        balloonContentBody: "Владимир, ул. Белоконской, 3/7",
                        balloonContentFooter: "+ 7(4922) 47-99-79"
                    });
                    myMap.geoObjects.add(myPlacemark);
                    myPlacemark.balloon.open();
                    window.innerWidth < 1115 ? document.getElementById("map").style.width = "100vw" : document.getElementById("map").style.width = "600px";
                    document.getElementById("map").style.height = "40vh";
                    window.innerWidth > 1115 ? document.getElementById("map").style.width = "600px" : document.getElementById("map").style.width = "100vw";
                ...
                map.geoObjects.each((function(geoObject) {
                    geoObject.balloon.events.add("open", (function(e1) {
                        ...
                        mapEventsGroup = map.events.group().add("actiontick", (function(e2) {
                            if ("outerBalloon" == placemark.options.get("balloonPane")) setBalloonPane(map, placemark, e2.get("tick"));
            ...
            if ("ФРЭМЭ`2022" == document.title) map();
            window["FLS"] = true;
            isWebp();
            addTouchClass();
            addLoadedClass();
